  ...
  export default {
    ...
    methods:{
      ...
      async handler(type,disNum,cart){
        // type:为了区分这三个元素 cart：哪一个产品 身上有产品
        // 向服务器发请求,修改数量
        switch(type){
          // 带给服务器变化的量
          case 'add':
          ...
          case 'minus':
            ...
            break
          case 'change':
            // if(isNaN(disNum) || disNum < 1 ){ // 输入进来是非法的  disNum不变化
            //   disNum = 0
            // }else{
            //   //      用户输入进来的量  -  商品本身的数量
            //   disNum = parseInt(disNum) - cart.skuNum
            // }
            disNum = (isNaN(disNum)||disNum < 1)? 0: parseInt(disNum) - cart.skuId
            break
        }
        try {
          // 成功的话等待 await
        await  this.$store.dispatch('addOrUpdateShopCart', {skuId:cart.skuId,skuInfo:disNum})
      // 上面是修改数据成功
      // 然后再一次获取服务器最新的数据进行展示
        this.getData()
      } catch (error) {
    ...
    computed:{
      ...
      // 简化后的购物车信息
      cartInfoList(){
        return this.cartList.cartInfoList || []  // 有可能是空对象
      },
      // 计算购买产品的总价格
      totalPrice(){
        let sum = 0
